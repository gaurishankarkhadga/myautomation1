// Mock C2D webhook test using PORT 8000
require('dotenv').config();
const axios = require('axios');

async function test() {
    console.log('Sending mock webhook to port 8000...');
    try {
        const payload = {
            object: 'instagram',
            entry: [{
        ...
        const res = await axios.post('http://localhost:8000/api/instagram/webhook', payload);
